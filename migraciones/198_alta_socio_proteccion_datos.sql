-- 198 · Alta de socio: tratamiento de datos (RGPD) + teléfono a INFO APOLANA
-- La contable (Isabel) revisó las altas y pidió: que conste la aceptación del
-- tratamiento de datos personales y la autorización a incorporar el teléfono
-- al grupo de difusión INFO APOLANA. Se guarda el texto aceptado, como con las
-- demás aceptaciones (normas, domiciliación, imagen).
alter table public.altas_socio
  add column if not exists acepta_proteccion_datos boolean,
  add column if not exists texto_proteccion_datos  text,
  add column if not exists telefono_info_apolana    boolean;
...
begin
  ...
  if coalesce((p ->> 'acepta_proteccion_datos')::boolean, false) is not true then
    return jsonb_build_object('ok', false, 'motivo', 'datos',
      'mensaje', 'Hay que aceptar el tratamiento de datos personales para hacerse socio.');
  end if;
  ...
  if public.texto_de_fuera(p ->> 'titular_cuenta', 140) is null then
  ...
  if coalesce((p ->> 'consiente_domiciliacion')::boolean, false) is not true then
    return jsonb_build_object('ok', false, 'motivo', 'domiciliacion',
      'mensaje', 'Hay que aceptar la orden de domiciliación para hacerse socio.');
  end if;
  ...
  insert into public.altas_socio (
    ...
    consiente_domiciliacion, texto_domiciliacion, iban, titular_cuenta,
    acepta_proteccion_datos, texto_proteccion_datos, telefono_info_apolana
  ) values (
    v_ref,
    ...
    public.texto_de_fuera(p ->> 'texto_domiciliacion', 1200),
    v_iban,
    public.texto_de_fuera(p ->> 'titular_cuenta', 140),
    coalesce((p ->> 'acepta_proteccion_datos')::boolean, false),
    public.texto_de_fuera(p ->> 'texto_proteccion_datos', 3000),
    coalesce((p ->> 'telefono_info_apolana')::boolean, true)
  );
